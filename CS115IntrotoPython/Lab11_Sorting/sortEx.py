...
def insertV1(L, i):
    ...
    while not (isSorted(L[:i + 1])):
        swap(L, i - numSwaps, i - numSwaps - 1)
        numSwaps += 1
...
def insertV2(L, i):
    ...
    saved = L[i]
    placeToMove = search(L, i, saved)
    for counter in range(0, i - placeToMove):
        swap(L, i - counter - 1, i - counter)
...
def letterCounts(fname):
    ...
    for element in content:
        element1 = letter(element)
        if element1 in dictionary:
            dictionary[(element1)] += 1
        else:
            dictionary[(element1)] = 1
    ourList = []
    for element in dictionary:
        ourList += [(dictionary[element], element)]
    for counter in range(0, len(ourList)):
        insertV2(ourList, counter)
    ourString = '['
    neededList = ourList[::-1]
    length = len(neededList)
    for i in range(0, length):
        if i == (length - 1):
            ourString += str(neededList[i]) + ']'
        else:
            ourString += str(neededList[i]) + ', '
    print(ourString)
