...
def fourBitAdd(xxxx,yyyy):
    ...
    and c is the carry.'''
    (c2, zo) = FA(xxxx[3], yyyy[3], 0)
    (c4, zt) = FA(xxxx[2], yyyy[2], c2)
    (c8, zf) = FA(xxxx[1], yyyy[1], c4)
    (c, ze) = FA(xxxx[0], yyyy[0], c8)
    return (c, (ze, zf, zt, zo))
...
def test_fourBitAdd():
    ...
    fifteen = (1, 1, 1, 1)
    fourteen = (1, 1, 1, 0)
    (c1, ww1) = fourBitAdd(zero, three)
    assert(gand(ww1 == (0, 0, 1, 1), c1 == 0))
    (c2, ww2) = fourBitAdd(seven, ten)
    assert(gand(ww2 == (0, 0, 0, 1), c2 == 1))
    (c3, ww3) = fourBitAdd(fifteen, fourteen)
    assert(gand(ww3 == (1, 1, 0, 1), c3 == 1))
    (c4, ww4) = fourBitAdd(ten, fourteen)
    assert(gand(ww4 == (1, 0, 0, 0), c4 == 1))
    print("four bit add works")
